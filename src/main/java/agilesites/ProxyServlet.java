...
public class ProxyServlet extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) {
		intProxyPort = SitesServer.getProxyPort();
		stringProxyHost = SitesServer.getProxyHost();
		stringProxyPath = SitesServer.getProxyPath();
		// Get the proxy host
		this.setProxyHost(stringProxyHost);
		// Get the proxy port if specified
		this.setProxyPort(intProxyPort);
		// Get the proxy path if specified
		this.setProxyPath(stringProxyPath);
	...
	private void executeProxyRequest(HttpMethod httpMethodProxyRequest,
		...
		if (intProxyResponseCode >= HttpServletResponse.SC_MULTIPLE_CHOICES /* 300 */
			...
			// proxied host
			String stringMyHostName = this.stringProxyHost; // httpServletRequest.getServerName();
			if (httpServletRequest.getServerPort() != 80) {
				stringMyHostName += ":" + intProxyPort; // httpServletRequest.getServerPort();
			}
			stringMyHostName += httpServletRequest.getContextPath();
			System.out.println("replacing " + getProxyHostAndPort()
					+ this.getProxyPath() + " with " + stringMyHostName);
